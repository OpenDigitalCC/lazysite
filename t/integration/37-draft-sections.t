...
sub clear {
    ...
    for my $f (qw(upcoming/pricing.html open/free.html index.html list.html)) {
        unlink "$docroot/$f" if -f "$docroot/$f";
    }

    # SM293 step 3: the sitemap is no longer a file in the docroot - it is
    # generated on request and CACHED for the registry TTL. Every subtest below
    # changes the ACLs and then asks what the sitemap says, so a cache left in
    # place would answer with the previous subtest's, and each assertion would
    # pass on the wrong document.
    File::Path::remove_tree("$docroot/lazysite/cache/registries")
        if -d "$docroot/lazysite/cache/registries";
    return;
}
...
# A sitemap is cached and served to the public, so a draft page listed in it is
# published regardless of what the page itself answers.
#
# SM293 step 3: fetched through the engine now rather than read off disk - it is
# generated on request and cached outside the document root. Fetching it is also
# the only way to assert what a crawler actually receives.
subtest 'a draft page is absent from the sitemap' => sub {
    write_acls( { 'upcoming' => { read => ['alice'], draft => JSON::PP::true } } );
    my $map = get('/sitemap.xml');
...
subtest 'a registry generated by an editor still excludes the draft' => sub {
    ...
    my $map = get_as( '/sitemap.xml', 'alice' );
    unlike( $map, qr{/upcoming/pricing},
