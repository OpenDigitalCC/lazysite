...
use lib "$FindBin::Bin/../../../lib";
use Lazysite::Manager::Upload qw(check_upload_rate);
...
$Lazysite::Manager::Upload::LAZYSITE_DIR = "$docroot/lazysite";
...
sub write_conf {
    ...
    print $fh $body;
    close $fh;
    Lazysite::Manager::Common::_reset_upload_limits_cache();
}
...
subtest 'allows request under count limit' => sub {
    ...
    reset_rate_db();
    for my $i ( 1 .. 3 ) {
        my $r = check_upload_rate( 'alice', 1024 );
...
subtest 'rejects when count limit reached' => sub {
    write_conf("manager_upload_rate_count: 2\n"
             . "manager_upload_rate_mb: 100\n");
    reset_rate_db();
    my $r1 = check_upload_rate( 'bob', 1024 );
    is( $r1->{ok}, 1, 'first request ok' );
    my $r2 = check_upload_rate( 'bob', 1024 );
    is( $r2->{ok}, 1, 'second request ok' );
    my $r3 = check_upload_rate( 'bob', 1024 );
...
subtest 'rejects when bytes limit would be exceeded' => sub {
    ...
    my $r1 = check_upload_rate( 'carol', $half_meg );
    is( $r1->{ok}, 1, 'first 0.5MB ok' );
    my $r2 = check_upload_rate( 'carol', $half_meg );
    is( $r2->{ok}, 1, 'second 0.5MB ok (exactly at limit)' );
    my $r3 = check_upload_rate( 'carol', 1 );
...
subtest 'both limits at 0 is a no-op' => sub {
    ...
    reset_rate_db();
    for my $i ( 1 .. 10 ) {
        my $r = check_upload_rate( 'dave', 999_999_999 );
...
subtest 'users share no budget' => sub {
    write_conf("manager_upload_rate_count: 2\n"
             . "manager_upload_rate_mb: 100\n");
    reset_rate_db();
    check_upload_rate( 'eve',  1024 );
    check_upload_rate( 'eve',  1024 );
    check_upload_rate( 'faye', 1024 );
    my $e = check_upload_rate( 'eve',  1024 );
    my $f = check_upload_rate( 'faye', 1024 );
...
subtest 'ages out buckets older than 2 hours' => sub {
    ...
    my $r = check_upload_rate( 'newcomer', 1024 );
