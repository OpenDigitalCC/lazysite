...
brand: plain
status: shipped
status-note: "SHIPPED 2026-08-17 as `action=actions-list`, plus a generated `docs/reference/control-api-actions.md`. The filing asked for it to be GENERATED from the dispatch table rather than hand-written, naming three defects already caused by hand-maintained lists - and there is no dispatch table: the control API is a 108-branch if/elsif chain, which SM237 met, worked around with a literal %KNOWN_ACTION, and said plainly deserves its own request. So Lazysite::ControlApi::Actions IS a fourth list; t/lint/58 is what stops it being a fourth defect. The table was EXTRACTED from the chain rather than typed, and the lint re-extracts it on every run and fails on any difference in the action set, the capabilities or the parameters. Replacing the chain with a real table remains the right fix and remains its own work. THE USEFUL PART is the three-state capability model the reference makes visible: any-one-of-these, any-authenticated (introspection), and COOKIE-ONLY - not reachable with a token at all, which is the state a caller can discover no other way and which covers 59 of the 111 actions. Subset by grant like tools/list (SM210), so a capless token sees only the three introspection actions rather than a menu of things to try and be refused. WHAT IT CANNOT SEE, recorded in the lint: a branch that hands the request to a helper reading a parameter internally is invisible to both the table and the extractor, so the reference is accurate about what it lists rather than exhaustive per action. AND ONE THING THE INSTALLER TAUGHT: adding lib/Lazysite/ControlApi/ broke every fresh install until the directory was declared in dist/config/classification.json - the installer refuses to create a directory above the site that it does not own, and says so clearly. A new module in a NEW subdirectory needs that declaration."
---
