...
brand: plain
status: shipped
status-note: "PARTIAL, 2026-08-17: the prerequisite and items 1-5 shipped; 6 and 7 did not. A session is bounded by thirty minutes of inactivity OR a day change - the day boundary matters independently, because a session straddling midnight would fold its exit page into the wrong day and the durable store is per-day. Sessions close on SILENCE as well as on a following event, so the last visit of a day records its exit rather than waiting for that visitor to come back. Everything stored is an aggregate: a counter on an edge, a bucket in a histogram, never anybody's path. Item 6 (device class) needs the user-agent threaded through both ingesters, which the batch record does not carry; item 7 (internal search terms) carries the only real privacy risk here and needs its own operator toggle and frequency floor, so it wants deciding rather than assuming. SHIPPED IN FULL 2026-08-17: items 6 and 7 landed too. Device class is three counters from the user-agent BOTH ingesters already had and both discarded - classify() consumed it and the record kept only its verdict - counted on page views only, since a stylesheet is fetched by the device that fetched the page and counting it would multiply every visit by however many files the layout loads (the SM329 rule). Tablet is tested BEFORE mobile because every Android tablet says 'Android' and only a phone also says 'Mobile'. Search terms ship OFF, on their own switch, top-20, with a frequency floor of 3 - and the floor is a privacy property rather than a reporting filter: below it only a HASH of the term is counted, so a term one visitor typed once exists on disk as twelve hex characters and nothing else. It is reversible for SM339's recount, and `search_terms` is ABSENT from a day file on a site that never enabled it, because an empty list reads as 'nobody searched' when the truth is 'nobody was asked'. AND A DEFECT FOUND ON THE WAY: the server-log parser captured the request TARGET with \\S+, query string included, and nothing downstream stripped it - _is_asset removed it for its own test and put it straight back. So /search-results?q=widgets and ?q=prices were two entries in top_pages, and every distinct search diluted the page counts of the page being searched from. Split once in the parser now. Pageview TOTALS are unchanged, so this is not a counting-basis change (SM338); only the top_pages keys aggregate differently, and a site with a busy search box will see its real pages rise."
---
