...
status: candidate
status-note: "FILED FROM AN INBOX BRIEF (archived at inbox/archive/), reported by the site agent 2026-08-26 against build 0.10.34 and RE-VERIFIED ON MAIN 2026-08-27: %CHANGES_ACCESS in lazysite-manager-api.pl still lists only acl-set, acl-remove, preview-grant and preview-clear. `domain-set` writes `allowed_groups` - the domain access model itself - and is absent from that list, so `actions-list` publishes it as an action that does not change access. TWO HALVES, DELIBERATELY SEPARATED, and the agent separated them: the declaration half is MEASURED and now confirmed in source; the scope-guard half is a CODE READING the agent explicitly marked as not demonstrated, because proving it would have required a live write to a real domain's access list - a grant-widening mutation, which they correctly declined to run on a shared instance on their own initiative. That refusal is the right call and the standing rule (never write a grant-widening mutation as a test, even with restore-on-exit); this filing keeps the two halves apart so nobody reads the unproved half as measured. THE DECLARATION HALF IS THE CHEAP ONE and should not wait for the other: a caller that consults changes_access to decide whether an action needs review is being told the wrong thing today."
---
