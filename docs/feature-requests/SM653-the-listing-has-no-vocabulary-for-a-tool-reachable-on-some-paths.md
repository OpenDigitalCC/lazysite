...
status: shipped
status-note: "SHIPPED. Both remedies are in. The listing SAYS the third case - a tool offered only through the path-aware theme/layout override carries a note in its description naming where it is callable, where it is refused, and which capability would widen it. And whoami now reports the two classes SEPARATELY (`tools_by_reach`: anywhere, path_only, and a sentence saying what path_only means), which was the second remedy and addresses SM525: an agent comparing what it holds against what it can call saw 26 tools it would be refused on every ordinary path, with nothing to separate them. The flat `tools` list is unchanged, so no existing reader has to change to keep getting what it already got. BOTH are DERIVED from _path_only_for, the same rule that decides callability - never a second list of tool names, which would be another place to keep in step and would be wrong the first time a tool gained a capability. The test holds the DERIVATION rather than the answer, and its first version could not fail: it matched the rule name across the whole file and found the sub DEFINITION rather than the call, passing with the call removed. Scoped to the classifier body, and sabotage-checked."
---
