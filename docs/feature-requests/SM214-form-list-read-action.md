...
brand: plain
status: shipped
status-note: "SHIPPED (0.10.1 edge). Both channels: the form-list control-API action AND the form_list MCP tool (Manager::Plugins::action_form_list; per form: name, handler_types, has_store, row count - PII-free; token-allowed; gated manage_forms|read_submissions on the API, read_submissions on MCP - the least-privilege cap, mirroring read_form_submissions). read_submissions unlocks now list form-list/form_list (capability-map regenerated). Plus the deliberate-exclusion comment on form-submission-delete (destructive PII stays interactive-only). PROPOSED 2026-07-27 by the lazysite.io site agent (inbox note, found reading cloudient.net submissions over the control API). Small, read-only, PII-free. Two parts: (1) a form-list control-API action + MCP counterpart gated manage_forms|read_submissions; (2) a decision on form-submission-delete's token exclusion (comment it as deliberate, or allow it for a manage_forms token client). Candidate for a stats/connector follow-up beta."
---
