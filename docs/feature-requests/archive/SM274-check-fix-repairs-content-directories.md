...
brand: plain
status: shipped
status-note: "SHIPPED 2026-08-11 (unreleased on main). NONE of options A, B or C as filed - a fourth. B was impossible as written: the install state records the DECLARED mode, not the mode the installer left, so drift cannot be told from deliberate by history without a new field that would only help future installs. But it does not need to be. GROUP WRITE is functional - without it the CGI cannot save - and nobody tightens a docroot subdirectory to break their own site on purpose; every other bit is preference. So --fix ADDS the group-write bit and nothing else, leaving world and owner bits exactly as the operator set them. A directory missing group write is now a FAIL rather than a difference. t/tools/36 pins both halves, including the one the hold existed for: a deliberately tightened 2770 directory is reported and left untouched. Two older subtests asserted the superseded behaviour and were updated rather than deleted, saying so. ORIGINAL: SPLIT from SM246 on 2026-08-11. SM246's four deliverables are complete - the incident explained, one declared permission model, the fresh-vs-upgrade policy declared, the CGI-writable file list declared once, and 0.10.5 added `check` verifying the whole declared directory set. This last third is BLOCKED ON A DECISION rather than on implementation, which is why it is its own filing: holding a four-deliverable request open for one undecided question misrepresents both."
---
