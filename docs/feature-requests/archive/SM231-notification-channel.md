...
brand: plain
status: shipped
status-note: "SHIPPED on main (unreleased) as the CHANNEL; the remaining endpoints and the agent notice store are [[SM281]]. Built: a type registry of seven (introspectable via notify_types; an UNREGISTERED type is still delivered on the generic template, because refusing it would lose an event whose caller landed before its registry entry); per-type per-endpoint templates overridable at lazysite/notify-templates/<type>.<endpoint>.tt, which is what finally DELIVERS url - it was recorded and discarded, and two of the three live callers were already passing one; routing via lazysite/notify.conf route.<type>, with the bell UNROUTABLE because it is the record - a route naming only xmpp still writes it; and emission control both site-wide (emit.<type>: off) and per form (notify: off in the form's own conf) for the 690-versus-5 case. A site that writes no notify.conf and no template behaves exactly as before: t/unit/lib/14-notify.t passes unchanged and is the compatibility statement. OPEN DECISION 2 ANSWERED: templates live in the CONFIG namespace, because a template in the theme namespace would travel in a site package and hand a client the agency's own alerting text. OPEN DECISION 1 (per-user addressing) deferred to SM281 with the SMTP endpoint and the readable notice store. Coverage of Lazysite::Notify went 56.7% (the weakest module in the tree) to 89.6%, 98.8% statements, via t/unit/lib/25-notify-channel.t and 26-notify-xmpp-transport.t - the latter drives the real _xmpp_send against a stubbed Net::XMPP, so the connect/auth/MUC-join sequencing and the disconnect-on-error path are executed rather than bypassed by the test hook. FILING CORRECTION: this document said notify had one caller and one type; by the time it was built there were three (submission, feedback, reset-request), which is itself the argument for a registry - nobody could see the set."
---
