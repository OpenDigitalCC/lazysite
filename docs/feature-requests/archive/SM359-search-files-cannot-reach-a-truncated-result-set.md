...
brand: plain
status: shipped
status-note: "SHIPPED 2026-08-17, with the priority INVERTED from the filing after measuring what is actually being paged. This is a depth-first walk of the site's own content tree reading a few hundred small text files - 181 on lazysite.io, 442 on dito.tech - against a 2,000-file budget and a 200-match cap. So the FILE budget essentially never fires on a real site and the MATCH cap fires constantly, which means the caller who hits truncation nearly always has too broad a QUERY rather than too small a page. The response could not tell them: both limits set the same bare boolean, and the two want opposite responses. `truncated_reason` (match_limit / file_budget) is therefore the primary fix and paging the secondary one. The `count` ambiguity needed no decision - it is scalar @matches, matches RETURNED - only the schema saying so. NO TOTAL: the scan stops AT the cap, so '200 of 1,431' means walking the whole tree and reading every file, which is the cost the cap exists to avoid; promising it would make the response slower in order to be more impressive. NO CURSOR: offset paging re-walks and can skip or repeat if the tree changes, said plainly in the schema rather than papered over - on a few hundred files the re-walk is milliseconds, and a cursor would mean inventing a stable index the LIFO traversal does not have. `truncated` means 'there is more after this page', not 'this page is full', which differ on exactly the page a caller stops on. TWO GAPS FOUND BY SABOTAGE: the file_budget reason was untested until a 2,100-file fixture was built for it (collapsing both reasons into one left the suite green), and the per-candidate is_blocked_path call in this walk is unreachable - the engine tree is skipped at descent and no dangerous extension is in the searchable set - so removing it leaves the tests passing. Both recorded in the test rather than papered over."
---
