...
brand: plain
status: shipped
status-note: "FILED 2026-08-17 while checking a claim I had just written. The CHANGELOG entry for SM336 items 6 and 7 said both fields reach the operator's Stats page; they reach its PAYLOAD. The page's own renderer has no sight of them, nor of the sessions and journeys shipped in the release before. Corrected the claim and filed this rather than leave a changelog asserting something a reader could check in one look. SHIPPED 2026-08-18. The page now renders devices, search terms, the visit count, the depth histogram, and entry and exit pages - and `sessions`, `entry`, `exit` and `depth` were added to the window projection to get there, since they had lived only in the day rollup. Two decisions carried through from the payload: the search-terms block is ABSENT rather than empty when the setting is off, because an empty list reads as 'nobody searched' and the truth is 'nobody was asked'; and the frequency floor is stated beside the terms, because an operator looking at what people typed should be told at that moment that a one-off never reaches the list. The setting note says so too, and the setting note is on a different page. THE ESCAPING IS THE SECURITY-RELEVANT PART: a search term is the first field this page renders whose content a STRANGER chooses, and it is escaped and deliberately NOT put in an href - pageTable does that because a page key IS a path, and copying the pattern would hand a stranger the target of a link in the manager UI. Both are sabotage-verified. What the test cannot establish is that the result LOOKS right, which is a manual check and is why the Stats chunk of the manual-check register still reads 'never'."
---
