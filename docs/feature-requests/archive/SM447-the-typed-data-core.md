...
D16 | Identifiers are QUOTED, and the header's claim was wrong | `SQLite.pm` argued its narrow `[a-z][a-z0-9_]*` pattern meant a name "needs no quoting in any dialect we generate for". True of every CHARACTER, false of a class of WORDS: `table`, `key`, `order`, `group`, `index` all match and are reserved, so `CREATE TABLE seating (table TEXT)` is a syntax error on an ordinary column. **Not a security fault** - the value was still bound - which is why it survived every test looking for injection, and why an integration test written for something else found it. Refusing the words instead would forbid ordinary column names forever AND carry SQLite's reserved list into engine-neutral code, which is the leak D11 exists to prevent. Quoting is the adapter's job.
D17 | The render path loads the data modules LAZILY | ADR 0001 keeps the processor module-free, and a `db:` page breaks that for itself. The alternative was a module-free reader hand-rolled in the processor, duplicating the descriptor loader, the coercion and the SQL generation - two readers of one store disagreeing about what a decimal is, invisible from either side. The ADR's reasoning is about a GATE on every request; this is a per-page opt-in, and a site that never writes `db:` loads nothing new.
D18 | A `db:` page is LIVE, never cached | It has no file whose mtime proves it current: the store is written through WAL, so a row can change without the database file's timestamp moving. Such a page is rendered per request. That costs something real, and costs less than serving last week's price list while reporting it fresh. A write-time cache invalidation would let these be cached again - **worth doing if edge shows the cost matters.**
D19 | `sqlite_see_if_its_a_number` is OFF | It converted stored TEXT `"120.00"` back to a Perl number - `"120.0"` - so every decimal lost a trailing zero on every read. Same fault as D8, one layer down and far worse. `"9.99"` round-trips through a number unchanged, so only a trailing zero exposes it, and every unit test built its own DBI handle rather than going through `Connect`. It took a price of 120.00 rendered on a page.
D20 | DP-6: a package carries data OPT-IN, a backup always | The release manager's call, 2026-08-22, and it narrows SM410's spec deliberately. A package is a portable hand-over artefact; shipping table contents by default hands a third party whatever an operator put in a directory or a contact table. Backups are for recovery and carry it unconditionally.
