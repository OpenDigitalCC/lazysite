...
brand: plain
status: shipped
status-note: "SHIPPED on main (unreleased) 2026-08-12, phase 1 item 3 of WORK-PLAN-ACCESS-CONTROL. A root entry is now the WEAKEST rule - checked only when nothing more specific governs - so 'everything private except the landing page' stays expressible, which is what makes a site-wide rule usable at all. '/' is canonical and what the writer stores; '', '.' and './' normalise to it, and the readers accept all of them because a hand-edited store is a real interface. GLOB spellings are REFUSED with a message naming '/': the store has no matching language anywhere else, so accepting '*' would imply one, and silently storing it is exactly what used to happen. Both readers changed identically (ADR 0001: the processor keeps a module-free copy; t/lint/31 pins the pair). FOUND BY THE WRITER TEST, not review: action_acl_remove did not understand the root key, so a site-wide rule could be created and NOT removed - a worse trap than not having one. lazysite-check now says which of the two shapes a site has, informational and never a warning, because enumerating folders is legitimate and nagging teaches the reader to skip the section. The protected-sections panel flags it site_wide and renders 'The whole site' rather than a folder called '/'. Verified failing on the stashed tree: 15 engine assertions and 25 writer assertions."
---
