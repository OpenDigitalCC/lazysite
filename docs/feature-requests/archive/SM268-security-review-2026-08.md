...
brand: plain
status: shipped
status-note: "SHIPPED in 0.10.5 (edge). Four adversarial reviews were run against 0.10.4 plus the unmerged SM195 branch, as a pre-release gate. 26 findings, every one reproduced by its reviewer, and the release was blocked until they were closed. ALL are now closed - 3 critical, 17 high (H17 was found while proving H15 and is in no report), 13 medium and 2 low - each with a regression test confirmed FAILING on the unfixed tree. Two carry a qualified status and the register says why beside them: M13 is MITIGATED rather than fixed, because reinterpreting an ACL key by content root would break the agreement between the four surfaces that write them, so it is documented and detected by audit_site instead; and M11 ships without the total-bytes cap the reviewer also asked for, because the number differs per host and a wrong default either fills the disk anyway or deletes an operator's snapshot on a small machine. M10 was fixed on claude/sm183-rollback, where its code lives. Full reports (2,048 lines, with reproductions) stay in tmp/security-review/ and are NOT committed - they carry live exploit detail."
---
...
| M10 | The `.sha256` sidecar is displayed but never verified. | 03-F10 | FIXED |
...
**M10 was fixed ON `claude/sm183-rollback`** (`3c4dd3d`), where the code lives -
`write_sha256`/`read_sha256` and the `-2` disambiguator are not on this branch.
Four things were wrong and all four are closed there: nothing verified the
digest (the listing now carries `sha256_status`, and `package_apply` refuses a
mismatch); the basename field was ignored, so a sidecar naming another artefact
passed; the sidecar was written non-atomically through a symlink-following
`open '>'` that returned a digest it had failed to store; and the installer's
own backups never got one, which made its retention `unlink` dead code.

That branch's `-2` disambiguator was replaced by the same `O_EXCL` claim used
here, so the two branches carry one mechanism rather than two answers to M9.
