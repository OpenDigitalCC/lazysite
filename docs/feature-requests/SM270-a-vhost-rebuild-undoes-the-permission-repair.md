...
brand: plain
status: shipped
status-note: "SHIPPED in 0.11.2 (edge, 2026-08-26). SHIPPED in 0.10.6 (4070fb5). All three items built: --rebuild orders template refresh, rebuild and deploy so the permission sweep runs last; lazysite-check reports a non-writable docroot as FAIL and repairs it under --fix (writable-but-no-setgid is a WARN, since a hand-made dev docroot is 0775 and failing on all of those teaches the reader to skip the line that matters); and the upgrade instructions say what a re-render costs. t/tools/36 covers both severities and the repair. ORIGINALLY FILED 2026-08-10 from a live 0.10.5 upgrade: the operator refreshed the Hestia template, rebuilt the vhost to pick up the SM268 H17 PT fix, and public_html came back drwxr-s--x (2751, no group write). Repaired by hand. This is the 0.6.5 incident class - the one SM246 exists for - reached by a route nothing checks. The 0.10.5 release notes and debian/changelog both say 'existing vhosts must be re-rendered' and NEITHER warns that re-rendering resets permissions."
---
