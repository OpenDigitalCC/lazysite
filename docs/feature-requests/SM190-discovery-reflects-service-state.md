...
brand: plain
status: shipped
status-note: "SHIPPED (all three parts). (1) oauth-* discovery 404s when oauth_enabled off - 0.9.12 (commit 0a300ff). (2) endpoint-coherence PROBE - lazysite-check #11 flags a remote service enabled with an unset/non-https/trailing-slash site_url, caught at deploy time without HTTP; 0.9.13, test in 04-check.t; shared with SM200 lever 4. (3) DEFERRED PART NOW DONE (2026-07-26, targeted at 0.9.16): .well-known/ai-partner is CODE-SERVED from the live config by the processor (_ai_partner_doc), intercepted before the render/cache path and served no-store, so it lists ONLY the endpoints whose killswitch is on (webdav/mcp/control_api/token_exchange) and cannot drift; it shadows any legacy static ai-partner.md (the starter page is now a registration stub for llms.txt). Built in Perl (hash -> JSON::PP) rather than TT to keep the JSON always-valid. Tests in 29-wellknown-oauth-gate.t; the canonical deny-list cross-check (06-deny-consistency.t) now reads the code-served list."
---
