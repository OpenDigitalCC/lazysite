...
status: candidate
status-note: "FILED FROM AN INBOX BRIEF (archived at inbox/archive/), reported by the site agent 2026-08-26. THE HEADLINE IS A PROVED NEGATIVE, and it is recorded here so it is not re-opened: `preview-grant` discloses no content, the stop is STRUCTURAL, and the mechanism is the right design and should not be changed. What the agent found instead is that the property holding it up is CALL ORDERING INSIDE ONE SUB - check_preview() sits below the auth gates - and NOTHING ASSERTS IT. t/unit/manager/09-preview-grant.t covers mint, verify, render, CSRF, bad layout/theme and no-store; t/integration/06-preview.t (re-checked on main 2026-08-27) carries three assertions and not one of them touches auth or an ACL. So a refactor that hoists preview context into an earlier resolution step turns this from safe into a content-disclosure hole SILENTLY, with the whole suite green. That test is worth more than everything else in this filing combined. SECONDARY, and cheap: preview-grant and preview-clear are two of only four entries in %CHANGES_ACCESS, whose own stated test is 'does the call alter who may read?' - for these two the answer is no, so they are OVER-flagged, the exact mirror of SM647's under-flagged domain-set. Same table, same session, opposite directions; worth one pass. A successful preview-grant also writes no audit row - dispatch exits before the audit_log call, and it sits in %skip (SM516 MA-15 already flags both entries as dead)."
---
