...
status: candidate
status-note: "RAISED 2026-08-27 by the operator. A login attempt is UNAUTHENTICATED by definition, so the name it carries is a claim, not an identity - but the auth surface writes that claim into the audit trail's actor column, and the Audit page renders every actor as a link to /manager/users?user=<name>. Anyone who can reach the login form can therefore put a string of their choosing into the operator's audit trail, have it listed as a distinct actor in the filter dropdown, and have it rendered as a live link to an account that does not exist. THE RULE THE OPERATOR SET: the actor is 'system' - the pseudo-actor this file already uses for ip-auto-blocked - and the attempted name goes in the detail field, sanitised. Nothing invented reaches the actor column, and nothing that is not an account is linked as one."
---
