...
status: candidate
status-note: "PROPOSED 2026-08-27 by the operator, as one set of actions because the parts do not work separately - renaming the group without fixing the role account leaves an account called `manager` sitting in the sysops group, and fixing the CLI actor without the vocabulary leaves the audit trail using words the documentation does not. THREE PRINCIPALS, ALREADY DISTINCT IN CODE AND COLLAPSED IN THE WORDS: a lazysite account with full capabilities working through the UI/API and subject to the capability model; a Unix account running the CLI, exempt from that model by construction ($actor eq 'local' bypasses _may_confer and _exceeds_authority) and logged as getpwuid($<); and no principal at all (`system`). `operator` has been used for the first two, which are not different privilege LEVELS but different KINDS of principal - one inside the model, one exempt from it. SETTLED: sysadmin = the host/CLI principal; sysop = the app's full-access principal, and it is a GROUP with named members; manager = the UI surface and nothing else; `operator` and `admin` retire. TWO OF THE THREE STRUCTURAL PARTS ARE DEFECTS RATHER THAN PREFERENCES - the actor-column collision and the role account - and are argued in the body. The operator ruled: name required, --link default, no alias for setup-manager ('lets forget the wrong way, not reinforce'), a deployed site with no accounts is a VALID state, and re-running setup-sysop with a different name creates another sysop. Related: SM641 (an actor is an account - this extends the same rule to the CLI and to first run), SM644 (sysops membership identifies the administrators, so a reset preserves it), SM093 (the one-command bootstrap this replaces)."
---
