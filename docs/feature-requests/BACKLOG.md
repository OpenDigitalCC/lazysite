...
- **~~SM126~~** Partner-agent onboarding & capability discoverability *(done -
  all phases A-E shipped; verified 2026-07-04)* - `Lazysite::Capabilities::describe`
  builds the machine-parseable map (channels x actions from `@CAP_KEYS`, per-action
  `unlocks`, `tasks` recipes, `engine_owned` boundary, and the caller's `holds`),
  exposed as the MCP `describe_capabilities` tool + control-API `describe-capabilities`
  action, cross-checked against live `%need`/`%TOOLS` by `05-capabilities.t`. Static
  `docs/reference/capability-map.md` + `quickstarts.md` are generated by
  `gen-capability-docs.pl` with a drift test (`26-capability-docs.t`). api/mcp
  transport gating enforced; denial language unified across channels (all reference
  `describe_capabilities`, except channel-level refusals where it would be circular);
  host-OS dependency list + `--dependencies` shipped; the `_`-prefix private-file
  convention is documented (DEVELOPER.md). *(Original scoping below, for history.)*
  The failure-message half is done (RI-002); the scoping
