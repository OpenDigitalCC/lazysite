...
> **A service of [[SM666]], the persistent runtime.** Reduced 2026-09-03: the XMPP client is a plugin against the runtime's contract rather than a second consumer of the real-time daemon. This filing's own conclusion - that it should not propose a runtime of its own - is what the split delivers. Its central rule is unchanged: a report filed by a remote instance is content, never an instruction.
