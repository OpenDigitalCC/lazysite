...
authoring, and `lazysite.conf` configuration without needing to explain the
system each time.
...
An optional `content_type` key sets the HTTP `Content-type` header. Default
...
Variables available on every page are defined in `lazysite/lazysite.conf`:
...
Define a nav structure in `lazysite.conf`:
...
Inline code elements (`\`like this\``) and fenced code blocks are protected
from TT processing - `[% tags %]` inside code appear literally, which is
...
Markdown link syntax `[text]([% url %])` does not work reliably because the
...
`lazysite/templates/registries/`. The template filename without `.tt` is the output
...
expires (default 4 hours - `$REGISTRY_TTL` in `lazysite-processor.pl`). To force
...
- All site-wide variables from `lazysite.conf`
...
Drop a `.tt` file in `lazysite/templates/registries/` - no code changes needed. The
...
TTL defaults to 1 hour - configured as `$REMOTE_TTL` in `lazysite-processor.pl`.
...
: The cache is invalidated by TTL (default 1 hour - `$REMOTE_TTL` in
  `lazysite-processor.pl`). The stale cache is always served immediately - the
...
Index pages (`index.md` / `index.html`) are served directly by Apache via
...
All non-index pages use extensionless URLs which never match a static file
directly, so they always go through the processor and trigger automatic
...
REDIRECT_URL=/install \
DOCUMENT_ROOT=/home/username/web/example.com/public_html \
  perl /home/username/web/example.com/cgi-bin/lazysite-processor.pl | grep -A5 -B5 'keyword'
```

### Syntax check the script

```bash
perl -c /home/username/web/example.com/cgi-bin/lazysite-processor.pl
```
...
Note: the `install-hestia.sh` hook sets this automatically when the template is
...
simulating the Apache CGI environment so that `lazysite.conf` variables like
...
`lazysite.conf` resolve to the correct values for the target deployment.
  ...
  web:
    ...
    command: >
      bash -c "
        ...
        cp /usr/lib/cgi-bin/lazysite-processor.pl /usr/lib/cgi-bin/ &&
...
`/usr/lib/cgi-bin/lazysite-processor.pl`.
...
```
lazysite/
  lazysite-processor.pl            <- CGI processor
  install.sh                       <- generic installer
  installers/
    hestia/
      install-hestia.sh            <- HestiaCP domain hook
      lazysite.tpl                 <- Apache vhost template (HTTP)
      lazysite.stpl                <- Apache vhost template (HTTPS)
    docker/                        <- placeholder for Docker setup
  starter/
    lazysite.conf                  <- starter site configuration
    index.md                       <- starter index page
    lazysite-demo.md               <- feature demonstrator
  ...
  test-site/                       <- local testing (gitignored content)
```
