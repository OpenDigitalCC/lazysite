...
2026-07-10 | Full-system disaster rehearsal (0.7.0 cut) | ~1 s mechanical restore | Full cycle from the shipped 0.6.10 tarball: install, operator content + manager account, `action_backup_create('full')`, docroot destroyed, `install.pl --restore-full --domain` onto a NEW docroot - content, auth store and conf verified intact. The mechanical restore is effectively instantaneous at starter-site scale; the 4 h RTO budget is operator response + DNS, not the mechanism. Script: rehearsal.sh (session records)
 ...
 2026-07-12 | Full-system disaster rehearsal (0.7.13 stable cycle) | <1 s mechanical restore | Same full cycle against the shipped 0.7.12 tarball - content, auth store and conf verified intact on the new docroot. Per-stable-cycle commitment met
 2026-08-26 | Full-system disaster rehearsal (0.11.0 stable cycle) | 69 ms mechanical restore | Full cycle against the SHIPPED 0.10.34 tarball, not the working tree - an operator recovers from the artefact, not the repository. Install, canary content + a manager account, `action_backup_create('full')`, docroot DESTROYED, `install.pl --restore-full --domain` onto a DIFFERENT docroot: content, auth store and `lazysite.conf` all verified intact. The 4 h RTO budget remains operator response + DNS, not the mechanism. NOTE, and it is the finding of this rehearsal: the backup is written INSIDE the docroot it backs up (`lazysite/backups/`), so the rehearsal had to copy it out before destroying the docroot - a real operator losing the docroot loses the backup with it unless it is already off the box. Script: tmp/rehearsal-0.11.0.sh (session records).
```
