...
# DA-26: the guard-load-hash lookup the three token questions share. Returns
# the record or undef; each caller keeps its own answer for "no such token",
# because they differ ('unknown' for token_status, undef for the other two).
sub _token_rec {
    my ($access) = @_;
    return undef unless defined $access && length $access;
    return load_store()->{tokens}{ _hash($access) };
}

# Resolve a presented access token to its partner, or undef if unknown/expired.
sub validate_token {
    my ($access) = @_;
    my $rec = _token_rec($access) or return undef;
    return undef if ( $rec->{exp} || 0 ) < _now();
    return $rec->{partner};
}
...
sub token_status {
    my ($access) = @_;
    my $rec = _token_rec($access) or return 'unknown';
