...
Notes:
  ...
  - The processor (cgi-bin/md-processor.pl) must be present relative
    to the docroot.
...
elif [ -f "./templates/layout.tt" ]; then
    DOCROOT="$(pwd)"
else
...
PROCESSOR="$DOCROOT/../cgi-bin/md-processor.pl"
if [ ! -f "$PROCESSOR" ]; then
    PROCESSOR="$DOCROOT/cgi-bin/md-processor.pl"
fi
if [ ! -f "$PROCESSOR" ]; then
    echo "Error: md-processor.pl not found." >&2
    echo "Expected at $DOCROOT/../cgi-bin/md-processor.pl or $DOCROOT/cgi-bin/md-processor.pl" >&2
    exit 1
fi
