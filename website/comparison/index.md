...
subtitle: Choose what to compare lazysite against.
register:
  - sitemap.xml
  - llms.txt
---
...
(function(){
  ...
  fetch(app.getAttribute('data-src')).then(function(r){return r.json();}).then(function(d){
    ...
    function cell(v,isLz){
      var cls=isLz?' class="lz"':'';
      var tip=(v&&v.source)?' title="Source: '+esc(v.source)+'"':'';
      return '<td'+cls+tip+'>'+(v?esc(v.text):'-')+'</td>';
    }
    function render(){
      ...
      var head='<thead><tr><th>Feature</th>'+cols.map(function(s){return '<th>'+esc(s.name)+'</th>';}).join('')+'</tr></thead>';
      var body='', lastG=null;
      d.items.forEach(function(it){
        if(it.group!==lastG){ body+='<tr class="cmp-grp"><th colspan="'+n+'">'+esc(it.group)+'</th></tr>'; lastG=it.group; }
        body+='<tr><th>'+esc(it.label)+'</th>'+cols.map(function(s){ return cell(it.values[s.key], s.key===lz); }).join('')+'</tr>';
      });
      out.innerHTML='<p class="cmp-hint">Hover any value to see the source it came from.</p><table class="cmp-tbl">'+cg+head+'<tbody>'+body+'</tbody></table>';
    }
    render();
