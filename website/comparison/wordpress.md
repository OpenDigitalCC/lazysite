---
title: lazysite vs WordPress
subtitle: The file-based engine versus the default CMS.
register:
  - sitemap.xml
  - llms.txt
tt_page_var:
  comp: json:/data/comparison-wordpress.json
---

WordPress is the default CMS - a database, a polished admin and a vast plugin ecosystem. lazysite sits at the other end: plain files, no database, built for speed and for AI-driven publishing.
...
**Choose lazysite** if you publish mostly text, want a smaller and safer stack, would rather own plain files, or want an AI agent to manage the content. **Choose WordPress** if you depend on a particular plugin, sell through a mature store, or need a non-technical team editing in a familiar admin. See the [other comparisons](/comparison).
