...
Control
: The layout template is a file you own and edit directly. The CSS is yours. The HTML structure is yours. lazysite renders Markdown into a slot in your template — it does not impose a theme, a component model, or a styling convention.
...
The registry system generates `llms.txt` and `sitemap.xml` from page front matter, updated automatically when pages render. Adding a new registry format requires only a template file.
...
The Docker staging workflow — a natural consequence of the file-based architecture. Stage in a container, rsync source files to production, let the cache warm on first visit.
